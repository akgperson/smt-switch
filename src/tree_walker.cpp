...
pair<Term, vector<int>> TreeWalker::visit(Term & node)
{
   ...
   * (giving last index in a node's path) or as a marker for when all the
   * children of the node have been visited: If the int is >=0, it indicates the
   * number for which child the term is and If the int is -1, it serves as a
   * flag that all children of the node have been visited and to go up a level
   * We first push back the pair (topmost node, -1) to to_visit, because when we
   * get back to it and pop it off, we know all the children of the topmost node
   * have been visited and the full formula has been traversed. Before entering
   * the loop, we also push back all the topmost node's children, so that they
   * and all their children can be processed before getting back to the topmost
  ...
  while (to_visit.size())
  {
    ...
    if (child_no != -1)
    {
      ...
      // child number for a term gives the last index in treepath, which is a
      ...
      // push back all children of current_term along with their respective
      // child numbers onto to_visit to visit in subsequent iterations
      child_no = 0;
      // push back all children of current_term we will need to visit before
      // popping all the way back to the current, parent term with the -1 flag
    ...
    {
      // child_no = -1, so this term and all the terms below it in the formula
